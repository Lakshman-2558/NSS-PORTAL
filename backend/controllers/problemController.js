...
exports.approveProblem = async (req, res) => {
  try {
    ...
    const event = await Event.create({
      ...
      date: eventDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // Default: 7 days from now
    ...
    await notifyAllStudentsAboutEvent(event, problem);
 ...
 */
async function notifyAllStudentsAboutEvent(event, problem) {
  try {
